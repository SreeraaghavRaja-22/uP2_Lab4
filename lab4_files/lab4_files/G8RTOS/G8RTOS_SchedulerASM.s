...
G8RTOS_Start:
	...
	; Disable interrupts
	CPSID I
	;PUSH {R4-R11} ; General Purpose Registers
	...
	ADD SP, SP, #4 ; discard LR from stack 
	POP {LR}
...
PendSV_Handler:
	...
	; Disable Interrupts
	CPSID I
	PUSH {R4-R11} ; Push R4 - R11 into the stack (others ones are done automatically)

	LDR R0, RunningPtr
	LDR R1, [R0]
	STR SP, [R1]

	; switches threads using the scheduler
	PUSH {R0, LR}

	; call scheduler to get new TCB
	BL G8RTOS_Scheduler
